...
I. To run the program on EMR. 
	...
	** Upload the required Jar files of Prediction.jar in the job folder of S3 bucket after compiling the JAVA file.
	** Make sure you have the input files uploaded in input folder in your bucket.
	** Enter your '{bucket_name}' with your bucket name in line 3 of Makefile.
	** Make sure HADOOP_CLASSPATH is set correctly before running any command.
		Command: export HADOOP_CLASSPATH=.:`hadoop classpath`

	** Make sure to run this command too:
		...
		4. Command to run Confusion Matrix: 
                  (Make sure to have a6validate file in confusion folder)
                  (cd to confusion folder and run following commands)
                        make run
                        make script
			
			
NOTE: If EMR is not working for you, try to check the parameters in Makefile, in line 3.
