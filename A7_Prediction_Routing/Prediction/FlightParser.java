...
public class FlightParser {
	...
	public static Flight getFlightData(String[] row, boolean isTest) throws IOException {
		...
		flight.setYear(Integer.parseInt(row[FlightConstants.INDEX_FLIGHT_YEAR + 1].trim()));
		flight.setMonth(Integer.parseInt(row[FlightConstants.INDEX_FLIGHT_MONTH + 1].trim()));
		flight.setDayOfMonth(Integer.parseInt(row[FlightConstants.INDEX_FLIGHT_DAY_OF_MONTH + 1].trim()));
		flight.setDayOfWeek(Integer.parseInt(row[FlightConstants.INDEX_FLIGHT_DAY_OF_WEEK + 1].trim()));
		
		flight.setCarrier(row[FlightConstants.INDEX_FLIGHT_CARRIER + 1].trim());
		flight.setFlightNum(Integer.parseInt(row[FlightConstants.INDEX_FLIGHT_NUM + 1].trim()));
		flight.setFlightDate(row[FlightConstants.INDEX_FLIGHT_DATE + 1].trim());
		
		flight.setOriginAirportId(Integer.parseInt(row[FlightConstants.INDEX_ORIGIN_AIRPORT_ID + 1].trim()));
		flight.setDestAirportId(Integer.parseInt(row[FlightConstants.INDEX_DEST_AIRPORT_ID + 1].trim()));
		
		if (!row[FlightConstants.INDEX_ACT_ARR_TIME].isEmpty() && !row[FlightConstants.INDEX_ACT_DEP_TIME].isEmpty()
				&& !row[FlightConstants.INDEX_ACT_ELAPSED_TIME].isEmpty()){
			
			flight.setCrsArrivalTime(Integer.parseInt(row[FlightConstants.INDEX_CRS_ARR_TIME + 1].trim()));
			flight.setCrsDepartureTime(Integer.parseInt(row[FlightConstants.INDEX_CRS_DEP_TIME + 1].trim()));
