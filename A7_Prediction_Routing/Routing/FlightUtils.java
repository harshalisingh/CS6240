...
public class FlightUtils {
	...
	/**
	 * Method to enforce the sanity test for every row (test data).
	 * Returns: Boolean value depending on whether a test flight passes
	 * sanity test or not.
	 */
	...
	public static boolean sanityTest(String[] row, boolean isTest){
		...
		try{
			...
			Integer month = Integer.parseInt(row[2]);
			
			Integer dayOfMonth = Integer.parseInt(row[3]);
			Integer dayOfWeek = Integer.parseInt(row[4]);
			String carrier = row[9];
			Integer flightNum = Integer.parseInt(row[10]);
			String flightDate = row[6];
			
			Integer originId = Integer.parseInt(row[11]);
			Integer destId = Integer.parseInt(row[20]);
			
			String origin = row[14];
			String dest = row[23];
			
			Integer crsDepartureTime = Integer.parseInt(row[29]);
			Integer crsArrivalTime = Integer.parseInt(row[40]);
			Integer crsElapsedTime = Integer.parseInt(row[50]);
			
			
			if(carrier == "NA" || flightDate == "NA" || origin == "NA" || dest == "NA"){
	...
	*/
	 ...
	 *            actualtime of arrival or departure in HHmm and delay in
	 *            minutes and returns time of arrival or departure in
	 *            milliseconds.
	 */
	
	public static long getScheduleTimeInMs(String flDate, int actualTimeOfDay, double delay) throws ParseException {
		Date date = convertToDate(flDate);
	...
	public static long minsToMs(String stime) {
	 ...
	 * milliseconds
	 */
	public static long calculateMs(String stime) {
