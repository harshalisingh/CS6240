...
import java.io._
/*
 * MedianComparison: The map reduce program that emits the median
...
object MedianComparison {
	def main(args:Array[String]) {
		...
		/*if (args.length != 2) {
			error("Usage: MedianComparison <input-path> <output-path>");
		}*/
		...
		println((t1 - t0)/1000);
		val fw = new FileWriter("testmedian.txt", true);
		try {
		...
		type Context = Reducer[Text, Text, Text, Text]#Context
		 ...
		 override def reduce(_K : Text, values: java.lang.Iterable[Text], context: Context) {
			...
			if(priceSize % 2 == 0){
				...
				medianPrice  = (prices(priceSize / 2) + prices((priceSize / 2) - 1)) / 2;
				
			} else {
				
				medianPrice = prices(priceSize / 2 - 1);
			}
			context.write(_K, new Text(count + "\t" + String.valueOf(medianPrice)));
		}
	}
