...
public class FastMedianMulti {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		FastMedianMulti fm = new FastMedianMulti();

		if(args.length == 1){
			System.out.println("Running Fast Median Parallel");
			int numThreads = files.length;
			WorkerThread[] threads = new WorkerThread[numThreads];
			for(int i =0; i < numThreads; i++){

				threads[i] = fm.new WorkerThread(files[i]);
				threads[i].start();
		...
		resultMap = computeFastMedian(carrierPriceList);
	...
	private static TreeMap<String, Float> sortValues(Map<String, Float> carrierFrequency2) {
		...
		SortValues sc = fm.new SortValues(carrierFrequency2);
	...
	private static void printResult(Map<String, Float> map, Map<String, Float> sorted){
		...
		for (Map.Entry<String, Float> entry : sorted.entrySet()) {
			...
			System.out.println(key + "\t" + entry.getValue() + "\t" +  map.get(key));
		}

	}
	
	@SuppressWarnings("unused")
	private static Map<String, Float> computeFastMedian(Map<String, ArrayList<Float>> carrierPriceList)
    {
		Map<String, Float> fastMedian = new HashMap<String, Float>();
		...
		for (Entry<String, ArrayList<Float>> entry : carrierPriceList.entrySet()) {
			...
			fastMedian.put(carrier, medianPrice);
		}
	...
	class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				synchronized (LOCK) {
					FastMedianMulti.K += corruptFlightCount;
					FastMedianMulti.F += saneFlightCount;
				}

				synchronized (FastMedianMulti.carrierPriceList) {
					updatePriceList(FastMedianMulti.carrierPriceList, cPriceList);
				}

				synchronized (FastMedianMulti.carrierFrequency) {
					updateFrequency(FastMedianMulti.carrierFrequency, cFrequency);
				}
