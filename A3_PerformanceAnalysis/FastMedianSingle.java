...
public class FastMedianSingle {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		FastMedianSingle ms = new FastMedianSingle();

		if(args.length == 1){
			System.out.println("Running FastMedian Single Threaded");
			for (File f : files) {
	...
	class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				synchronized (LOCK) {
					FastMedianSingle.K += corruptFlightCount;
					FastMedianSingle.F += saneFlightCount;
				}

				synchronized (FastMedianSingle.carrierPriceList) {
					updatePriceList(FastMedianSingle.carrierPriceList, cPriceList);
				}

				synchronized (FastMedianSingle.carrierFrequency) {
					updateFrequency(FastMedianSingle.carrierFrequency, cFrequency);
				}
