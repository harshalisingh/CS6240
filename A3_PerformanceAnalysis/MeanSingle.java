...
public class MeanSingle {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		MeanSingle ms = new MeanSingle();

		if(args.length == 1){
			System.out.println("Running Mean Single Threaded");
			for (File f : files) {
	...
	class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				synchronized (LOCK) {
					MeanSingle.K += corruptFlightCount;
					MeanSingle.F += saneFlightCount;
				}

				synchronized (MeanSingle.carrierPriceList) {
					updatePriceList(MeanSingle.carrierPriceList, cPriceList);
				}

				synchronized (MeanSingle.carrierFrequency) {
					updateFrequency(MeanSingle.carrierFrequency, cFrequency);
				}
