...
public class MedianMulti {
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		MedianMulti ms = new MedianMulti();

		if(args.length == 1){
			System.out.println("Running Median Parallel");
			int numThreads = files.length;
			WorkerThread[] threads = new WorkerThread[numThreads];
			for(int i =0; i < numThreads; i++){

				threads[i] = ms.new WorkerThread(files[i]);
				threads[i].start();
	...
	class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				synchronized (LOCK) {
					MedianMulti.K += corruptFlightCount;
					MedianMulti.F += saneFlightCount;
				}

				synchronized (MedianMulti.carrierPriceList) {
					updatePriceList(MedianMulti.carrierPriceList, cPriceList);
				}

				synchronized (MedianMulti.carrierFrequency) {
					updateFrequency(MedianMulti.carrierFrequency, cFrequency);
				}
